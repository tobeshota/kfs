...
int register_unit_tests(struct kfs_test_case **out)
{
	...
	if (!all_cases)
	{
		...
		struct kfs_test_case *cases_exit = 0;
		int count_exit = register_unit_tests_exit(&cases_exit);
		struct kfs_test_case *cases_wait = 0;
		...
		struct kfs_test_case *cases_ext = 0;
		int count_ext = register_unit_tests_ext(&cases_ext);
		...
		struct kfs_test_case *cases_exec = 0;
		int count_exec = register_unit_tests_exec(&cases_exec);
		struct kfs_test_case *cases_mmap = 0;
		int count_mmap = register_unit_tests_mmap(&cases_mmap);
		struct kfs_test_case *cases_piano = 0;
		int count_piano = register_unit_tests_piano(&cases_piano);
		struct kfs_test_case *cases_socket = 0;
		int count_socket = register_unit_tests_socket(&cases_socket);
		...
		for (int i = 0; i < count_exit && idx < KFS_MAX_TESTS; i++)
		{
			merged[idx++] = cases_exit[i];
		}
		for (int i = 0; i < count_wait && idx < KFS_MAX_TESTS; i++)
		{
			merged[idx++] = cases_wait[i];
		}
		...
		for (int i = 0; i < count_ext && idx < KFS_MAX_TESTS; i++)
		{
			merged[idx++] = cases_ext[i];
		}
		for (int i = 0; i < count_process && idx < KFS_MAX_TESTS; i++)
		{
			merged[idx++] = cases_process[i];
		}
		for (int i = 0; i < count_pcspkr && idx < KFS_MAX_TESTS; i++)
		{
			merged[idx++] = cases_pcspkr[i];
		}
		...
		for (int i = 0; i < count_exec && idx < KFS_MAX_TESTS; i++)
		{
			merged[idx++] = cases_exec[i];
		}
		for (int i = 0; i < count_mmap && idx < KFS_MAX_TESTS; i++)
		{
			merged[idx++] = cases_mmap[i];
		}
		for (int i = 0; i < count_piano && idx < KFS_MAX_TESTS; i++)
		{
			merged[idx++] = cases_piano[i];
		}
		for (int i = 0; i < count_socket && idx < KFS_MAX_TESTS; i++)
		{
			merged[idx++] = cases_socket[i];
		}
		all_cases = merged;
		all_count = idx;
	}
