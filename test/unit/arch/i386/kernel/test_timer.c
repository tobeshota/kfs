...
KFS_TEST(test_timer_init_and_latch_readback)
{
	...
	for (int i = 0; i < 3; i++)
	{
		/** ラッチコマンド（チャネル 0 のカウント値を内部レジスタに「固定（ラッチ）」）
		 * 0x00 = 0000 0000
		...
		/* カウント値を lobyte → hibyte の順で読み取る */
		unsigned int lo = inb(PIT_CHANNEL0_PORT); // 内部レジスタがラッチ(固定)された値の下位8bit
		unsigned int hi = inb(PIT_CHANNEL0_PORT); // 内部レジスタがラッチ(固定)された値の上位8bit
		counts[i] = (hi << 8) | lo;
